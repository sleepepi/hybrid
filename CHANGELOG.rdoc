...
* Enhancements
  ...
  * The data source popup box now displays:
    ...
    * Associated Ontologies: Lists of the ontologies that have concepts mapped to the data source
...
* Documentation
  * Added a sample ontology file (CSV) to doc/ontology/tiny_ontology.csv
  * Added a sample database file (SQL Dump) to doc/ontology/tiny_sleep.sql
...
* Testing
  ...
  * Ontology controller functional test now tests that CSV files are imported correctly
...
* Enhancements
  ...
  * Data Sources can now generate their derived mappings to leverage relationships across and within ontologies
  * Sensitive variables can now be identified in the ontology and can only be viewed by users with source access rule:
